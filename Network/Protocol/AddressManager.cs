...
namespace NBitcoin.Protocol
{
	...
	public class AddressManager
	{
...
#if LOCAL_TESTS
		const int ADDRMAN_GETADDR_MAX_PCT = 100;
#else
...
#if LOCAL_TESTS
		AddressInfo Find(IPEndPoint addr)
#else
...
#if LOCAL_TESTS
		AddressInfo Find(IPEndPoint addr, out int pnId)
#else
...
#if LOCAL_TESTS
		Dictionary<IPEndPoint, int> mapAddr = new Dictionary<IPEndPoint, int>();
#else
...
#if LOCAL_TESTS
		public IEnumerable<IPEndPoint> All() {
			return mapAddr.Keys;
		}
#endif
...
#if LOCAL_TESTS
			mapAddr.Remove(info.Address.Endpoint);
#else
		...
		{
#if LOCAL_TESTS
			return Find(addr.Endpoint, out nId);
#else
		...
		private int peerToFind = 1000;
		public int PeersToFind {
			get { return peerToFind; }
			set { peerToFind = value; }
		}

		internal void DiscoverPeers(NetworkInfo network, NodeConnectionParameters parameters)
		{
				...
				while(found < peerToFind)
				{
					...
					try
					{
						...
						}, p =>
						{
							...
							{
								NodeServerTrace.Error($"Discovery error with peer: {p}", e);
							}
							finally
							{
								if(n != null)
		...
		//private static void PopulateTableWithDNSNodes(NetworkInfo network, List<NetworkAddress> peers)
		//{
		...
		private static void PopulateTableWithHardNodes(NetworkInfo network, List<NetworkAddress> peers)
		{
