...
let emptySMT<'V> = fun cTW -> {cTW=cTW; kv= Map.empty<Hash,'V>; digests = ref Map.empty<Loc,Hash>; defaultDigests = defaultHash cTW}
...
let split ({kv=kv} as smt:SMT<'V>) s =
    let kvl, kvu = splitm kv s
...
let ihopt defdigests dl dr b h =
    ...
    | None, None -> None
    | None, Some r ->
        Some <| innerHashList [defdigests (h-1); r; bitsToBytes b; toBytes h]
    | Some l, None ->
        Some <| innerHashList [l; defdigests (h-1); bitsToBytes b; toBytes h]
    | Some l, Some r ->
...
let rec updateSMTOpt (serializer:'V->byte[]) (rootSmt : SMT<'V>) ({cTW=cTW;kv=kv;digests=digests;defaultDigests=defaultDigests}:SMT<'V> as smt) ({height=height;loc=loc} as location:Loc) (kvs:Map<Hash,'V option>) =
    if height=0 then
        let newval = kvs.TryFind <| bitsToBytes loc
        printfn "height=0; newval = %A; loc = %A; smt.kv=%A" newval (bitsToBytes loc) (smt.kv)
        match newval with
        | Some (Some v) -> rootSmt.kv <- rootSmt.kv.Add (bitsToBytes loc, v)
        | Some None -> 
            printfn "removing at loc %A, current value is %A" (bitsToBytes loc) (rootSmt.kv.TryFind <| bitsToBytes loc)
            rootSmt.kv <- rootSmt.kv.Remove (bitsToBytes loc)
        | None -> ()
        printfn "new rootSmt.kv.[loc] is %A" (rootSmt.kv.TryFind <| bitsToBytes loc)
        lhopt serializer cTW loc <| Option.bind id newval
    else
